...
class View(BaseView):
    ...
    def _get_extensions_flatpak(self):
        ...
        try:
            ...
            extensions_iface = dbus.Interface(shell_object, 'org.gnome.Shell.Extensions')
    ...
    def load_extensions(self):
        ...
        try:
            ...
            for uuid, data in shell_extensions.items():
                ext_data = {
                    'uuid': uuid,
                    'name': data.get('name', uuid.split('@')[0].replace('-', ' ').title()),
                    'description': data.get('description', '').split('\n')[0],
                    'state': ExtensionState.ENABLED if uuid in enabled_extensions else ExtensionState.DISABLED,
