...
class View(BaseView):
    ...
    def on_add_clicked(self, button):
        ...
        def create_app_page():
            ...
            add_button.connect("clicked", lambda b: self.on_add_button_clicked(b, dialog))
            ...
            def load_applications():
                ...
                if is_flatpak():
                    with tempfile.TemporaryDirectory() as temp_dir:
                        ...
                        host_apps_cmd = "ls /usr/share/applications/*.desktop 2>/dev/null"
                        result = run_command(host_apps_cmd, shell=True)
                        
                        if result:
                            desktop_files = result.strip().split('\n')
                            logger.debug(f"Found {len(desktop_files)} host applications")
                                ...
                                try:
                                    ...
                                    if content and '[Desktop Entry]' in content:
                                        ...
                                        for line in content.split('\n'):
                                            ...
                                            if line.startswith('DBusActivatable=') or \
                                            ...
                                            if line.startswith('Name='):
                                                ...
                                                if '/' in icon_name:
                                                    ...
                                                    if '.' in icon_name:  # Remove extension if present
                                        ...
                                        entry = DesktopEntry(desktop_file, final_content)
                                        
                                        if entry.should_show():
                                            print(f"Adding host app: {entry.get_name()}")
                                            row = create_app_row(entry)
                                            list_box.append(row)
                                
                                except Exception as e:
                                    logger.error(f"Error processing host app {desktop_file}: {e}", exc_info=True)
                        ...
                        for location in flatpak_locations:
                            ...
                            if result and result != 'true':
                                desktop_files = result.strip().split('\n')
                                logger.debug(f"Found {len(desktop_files)} Flatpak applications in {location}")
                                for desktop_file in desktop_files:
                                    if not desktop_file.strip():
                                        continue
                                    ...
                                    try:
                                        ...
                                        if content and '[Desktop Entry]' in content:
                                            ...
                                            if has_name and has_type:
                                                ...
                                                temp_path = os.path.join(temp_dir, os.path.basename(desktop_file))
                                                with open(temp_path, 'w') as f:
                                                    f.write('\n'.join(processed_content))
                                                
                                                # Try to create app info from the modified file
                                                app_info = Gio.DesktopAppInfo.new_from_filename(temp_path)
                                                if app_info and app_info.should_show():
                                                    print(f"Adding Flatpak app: {app_info.get_name()}")
                                                    row = create_app_row(app_info)
                                                    list_box.append(row)
                                    
                                    except Exception as e:
                                        logger.error(f"Error processing Flatpak app {desktop_file}: {e}", exc_info=True)
                else:
    ...
    def on_add_button_clicked(self, button, dialog):
        ...
        if hasattr(self, "selected_app") and self.selected_app:
            logger.info(f"Adding application to startup: {self.selected_app.get_name()}")
                ...
                for line in content.split('\n'):
                    if line.startswith('Exec='):
                        ...
                        if 'flatpak run' in line:
                            ...
                            processed_content.append(line)
                    elif not (line.startswith('DBusActivatable=') or 
                             line.startswith('X-') or 
                             line.startswith('Actions=')):
                        processed_content.append(line)
