...
def run_command(command, shell=False):
    ...
    if is_flatpak():
        if isinstance(command, str):
            if shell:
                ...
                if command.startswith("dconf write"):
                    ...
                    if len(parts) == 3:
                        ...
                        full_command = ["flatpak-spawn", "--host", "dconf", "write", key, value]
                    else:
                    ...
                    parts = command.split()
                    if len(parts) == 3:
                        full_command = ["flatpak-spawn", "--host", "dconf", "reset", parts[2]]
                    else:
                        full_command = ["flatpak-spawn", "--host", "bash", "-c", command]
                else:
                    full_command = ["flatpak-spawn", "--host", "bash", "-c", command]
            ...
            if isinstance(command, list) and len(command) > 0 and command[0] == "dbus-send":
    ...
    try:
        logger.debug(f"Running command: {' '.join(full_command if isinstance(full_command, list) else [full_command])}")
